## Маркетплейс SkyMarketPlace 🌥️
### 1) Назва застосунку 👽
**SkyMarketPlace** - це унікальний онлайн-маркетплейс, створений для тих, хто хоче вийти за межі звичних онлайн-магазинів. Це платформа, де можна не лише знайти «все, що попало», а й продати будь-що за вигідною ціною. Вона ідеально підходить для користувачів, які цінують гумор, свободу, спонтанність і шукають нові формати онлайн-торгівлі.
### 2) Мета проект😇
...
+ User:
  ...
  - пароль;
  - роль користувача (адміністратор/ покупець);
...
+ Створення, редагування та видалення оголошень адміністратором;
...
### 7) MySQL 🗄️
+ Популярна та надійна система керування базами даних;
+ Забезпечує зберігання та обробку даних для:
  + збереження інформації про товари та оголошення;
  + обліку користувачів і їхніх ролей;
  + зберігання замовлень та їх статусів;
  + фільтрації та вибірки даних за заданими критеріями.

### 8) Контрольні запитання ЛР1
...
## 12) Контрольні запитання ЛР6
#### *1. Що таке серіалізація і для чого вона використовується у C#?*
**Серіалізація** *(Serialization)* - це процес перетворення об’єкта у формат, який можна зберегти у файл або передати по мережі. Зазвичай це текстовий (JSON, XML) або бінарний формат. Наприклад: об’єкт типу Cat зберігається у файл cats.json у вигляді тексту.
#### *2. Що таке десеріалізація? У яких випадках вона потрібна?*
**Десеріалізація** *(Deserialization)* - це процес зчитування збереженого формату (наприклад, JSON ) та відновлення об’єкта у пам’яті програми. Наприклад: зчитування cats.json і створення об'єктів Cat у списку List<Cat> .
#### *3. Які формати серіалізації підтримуються в C#? У чому різниця між JSON та XML?*
Основні формати серіалізації в C#:
- JSON (найпопулярніший)
- XML
- Binary (бінарна серіалізація)
- YAML (додаткові бібліотеки)
- Protocol Buffers (від Google)
Різниця між JSON та XML:
- **JSON** – коли важливі простота, швидкість, веб-розробка.
- **XML** – коли важлива точність опису складних структур, додаткові метадані або підтримка старих систем.
#### *4. Що таке інтерфейс IDataStorage<T> і яку роль він виконує у проєкті?*
**Інтерфейс IDataStorage<T>** — це абстракція (тобто узагальнене представлення), яка визначає набір методів для роботи з даними. Зазвичай використовується, коли у проєкті є кілька способів зберігання чи доступу до інформації, і потрібно мати єдиний, стандартизований спосіб взаємодії з цими різними джерелами.
Інтерфейс IDataStorage<T> робить проєкт гнучким і модульним, осканлюючи конкретні способи роботи з даними від загальної логіки застосунку.
Наприклад, у вас є кілька сховищ даних:
- файл
- база даних
- хмарне сховище
Якщо всі ці сховища реалізують інтерфейс IDataStorage<T>, код застосунку може легко перемикатися між різними сховищами, не змінюючи основний функціонал.
#### *5. Чому доцільно відокремлювати логіку збереження ( IDataStorage<T> ) від логіки доступу ( GenericRepository<T> )?*
Відокремлювати логіку збереження даних (IDataStorage<T>) від логіки доступу до даних (GenericRepository<T>) доцільно для того, щоб зробити код більш гнучким, зрозумілим і легким у підтримці.
#### *6. Чому метод Save() повинен викликатись після кожної зміни ( Add , Update , Delete )?*
Метод Save() повинен викликатися після кожної зміни (Add, Update, Delete), тому що всі ці зміни зазвичай відбуваються лише в оперативній пам'яті (в тимчасовому сховищі). Щоб ці зміни були збережені постійно (наприклад, у файлі чи базі даних), їх потрібно явно зафіксувати.
#### *7. Як працює GenericRepository<T> з джерелом зберігання JsonStorage<T>?*
- GenericRepository<T> просто каже: «Збережи мені ці дані!».
- JsonStorage<T> бере ці дані і перетворює в JSON-файл.
- Завдяки такій структурі репозиторій не залежить від конкретного способу зберігання. Можна легко перейти з JSON на базу даних або навпаки, просто замінивши сховище.
